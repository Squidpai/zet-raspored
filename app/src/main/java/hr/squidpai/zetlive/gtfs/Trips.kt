...
class Trips(
  ...
  fun printTripShapes(stops: Stops) {
    for (shape in tripShapes) {
      if (shape.first().toStopId() != commonFirstStop.first && shape.first().toStopId() != commonFirstStop.second)
        continue
...
fun TripsList.filterByDirection(): Pair<TripsList, TripsList> {
...
class MutableRoutesAtStop(
  ...
  constructor(first: Boolean, last: Boolean, firstRoute: Int) : this(first, last, mutableIntSetOf(firstRoute))
...
object TripsLoader {
  ...
  private fun calculateAllCommonHeadsigns(zippedTrips: ZippedTrips): IntObjectMap<Pair<String, String>> {
    val appearances = MutableIntObjectMap<Pair<MutableObjectIntMap<String>, MutableObjectIntMap<String>>>()
  ...
  private fun DataInputStream.readTrip(tripShapes: List<IntList>): Trip {
    ...
    return Trip(routeId, serviceId, tripId, headsign, directionId, blockId, stops, departures, tripShape)
  }
