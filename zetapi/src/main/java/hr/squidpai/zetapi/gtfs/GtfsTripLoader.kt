...
internal class GtfsTripLoader private constructor(
    ...
    companion object {
        fun loadTrips(
            ...
            val loader =
                GtfsTripLoader(zip, routes, stops, shapes, realtimeDispatcher, onLoadProgress)
            loader.loadTrips()
    ...
    private class GtfsTrip(
        ...
        fun toTrip(
            ...
            blockId,
            shape ?: run {
                val fauxShapeId = "${route.id}_${stops.hashCode().toHexString()}"
                fauxShapes.getOrPut(fauxShapeId) {
                    Shape(
                        fauxShapeId,
                        stops.map { listOf(Shape.Point(it.latitude, it.longitude, Float.NaN)) }
                    )
                }
            },
            stops,
