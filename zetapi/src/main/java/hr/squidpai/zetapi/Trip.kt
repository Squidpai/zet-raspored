...
public class Trip internal constructor(
	...
	public fun getRealtimeDepartures(): TimeOfDayList? {
		return when (val data = realtimeDispatcher.getForTrip(tripId)) {
			is RealtimeData.DelayByStop -> {
				val realtimeDepartures = MutableIntList(departures.size)

				departures.forEachIndexed { i, departureInSeconds ->
					val departure = TimeOfDay(departureInSeconds)

					realtimeDepartures += (departure + data[i]).valueInSeconds
				}
